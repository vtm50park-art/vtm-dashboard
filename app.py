# ═══════════════════════════════════════════════════════════════════
#  VTM OS 로그인 리디자인 패치 (v2.0.0)
#  아래 3개 블록을 vtm_dashboard.py 의 같은 이름 함수와 "통째로" 교체하세요.
#    1) init_data()     ← 서아영(emp_seo) 신규 생성 제거 + 기존 DB 퇴직 처리
#    2) inject_all()    ← 기존 전역 CSS는 그대로, 로그인 화면 전용 CSS만 추가
#    3) render_login()  ← 레퍼런스 시안 기반 리디자인 (로그인 로직은 원본 그대로)
#
#  ※ Supabase 연결, get_employees, 비밀번호 검증, session_state, st.rerun 등
#     기능 코드는 단 한 줄도 변경하지 않았습니다.
# ═══════════════════════════════════════════════════════════════════

VTM_LOGO_URL  = "https://i.postimg.cc/TwMLPgWj/beu-itiem-logo.png"
VTM_VIMEO_ID  = "1206450035"


# ───────────────────────────────────────────────────────────────────
# 1) init_data()  ─ 교체 블록
#    변경점: emp_seo(서아영 디자이너) insert 항목 삭제
#            + 기존 DB에 emp_seo가 남아 있으면 active=0 퇴직 처리
# ───────────────────────────────────────────────────────────────────
def init_data():
    ...
    try:
        ...
        #    (근태/보고 기록이 user_id로 연결되어 있을 수 있어 삭제 대신 active=0 권장.
        #     완전 삭제를 원하면 Supabase SQL Editor에서 별도 실행 — 안내문 참조)
        try:
...
# ───────────────────────────────────────────────────────────────────
# 2) inject_all()  ─ 교체 블록
#    변경점: 기존 전역 CSS/별 캔버스는 원본 그대로 유지.
#            로그인 전(=logged_in False)에만 적용되는
#            "VTM OS 로그인 전용 CSS" 블록을 뒤에 추가.
#            (로그인 후에는 자동으로 원래 골드 테마로 복귀)
# ───────────────────────────────────────────────────────────────────
def inject_all():
...
""", unsafe_allow_html=True)
    ...
    # ═══════════════════════════════════════════════════════════
    # ▼▼▼ [추가] VTM OS 2.0 로그인 전용 CSS ▼▼▼
    #     로그인 전(logged_in=False)에만 주입 → 로그인 후 다른 페이지
    #     스타일에는 어떠한 영향도 주지 않음.
    # ═══════════════════════════════════════════════════════════
    if not st.session_state.get("logged_in", False):
        st.markdown("""
<style>
/* ── 로그인 모드: 배경 투명화 (Vimeo 영상이 비치도록) ── */
html, body, .stApp,
[data-testid="stAppViewContainer"],
[data-testid="stAppViewContainer"]>section,
[data-testid="stMain"],
...
/* ── Vimeo 배경 영상 (fixed, cover) + 어두운 오버레이 ── */
.vtm-bgvid iframe {
...
.vtm-bgoverlay {
    ...
    background:
        radial-gradient(ellipse at 28% 40%, rgba(20,224,184,0.10) 0%, transparent 55%),
        linear-gradient(180deg, rgba(3,9,18,0.78) 0%, rgba(3,9,18,0.68) 45%, rgba(3,9,18,0.85) 100%);
}
...
/* ── 좌측 브랜드 영역 ── */
.vtm-brand { padding: 10px 28px 10px 4px; }
.vtm-brand-logo img {
    width: 168px; max-width: 46vw; height: auto; display: block;
    filter: drop-shadow(0 0 28px rgba(20,224,184,0.35));
    margin-bottom: 22px;
}
.vtm-brand-title {
    color: #E8FFFA; font-size: 2.55rem; font-weight: 900;
...
.vtm-brand-sub {
    color: #5EEAD4; font-size: 1.02rem; font-weight: 700;
    letter-spacing: 0.44em; margin: 8px 0 0 2px;
}
.vtm-brand-tag {
    color: #B7C6D9; font-size: 0.94rem; font-weight: 500;
    margin: 22px 0 0; letter-spacing: 0.02em;
}
.vtm-feat-row {
    display: flex; gap: 34px; margin-top: 38px; flex-wrap: wrap;
}
.vtm-feat { text-align: center; min-width: 88px; }
.vtm-feat svg { width: 30px; height: 30px; margin-bottom: 8px; }
.vtm-feat-t { color: #F1F5F9; font-size: 0.86rem; font-weight: 800; margin: 0; }
.vtm-feat-d { color: #7E93AB; font-size: 0.76rem; font-weight: 500; margin: 3px 0 0; }
.vtm-copy {
    color: #55677E; font-size: 0.74rem; font-weight: 500; margin-top: 52px;
}

/* ── 우측 로그인 카드 (글래스모피즘) ── */
[data-testid="stColumn"]:has(.vtm-login-marker),
[data-testid="column"]:has(.vtm-login-marker) {
    background: linear-gradient(165deg, rgba(14,26,44,0.72) 0%, rgba(7,14,26,0.86) 100%);
...
.vtm-card-head { text-align: center; margin-bottom: 18px; }
.vtm-card-head img {
    width: 58px; height: auto;
    filter: drop-shadow(0 0 14px rgba(20,224,184,0.45));
}
.vtm-card-os {
    color: #2DD4BF; font-size: 1.02rem; font-weight: 900;
    letter-spacing: 0.24em; margin: 10px 0 4px;
}
.vtm-card-welcome {
    color: #FFFFFF; font-size: 1.72rem; font-weight: 900; margin: 0;
}
.vtm-card-sub {
    color: #8CA3BC; font-size: 0.86rem; font-weight: 500; margin: 8px 0 0;
}
.vtm-nopw {
...
.vtm-ver {
    text-align: center; color: #5E7490; font-size: 0.76rem;
...
/* ── 별 캔버스: 영상 위에 은은하게 유지 ── */
#vtm-stars { opacity: 0.3 !important; z-index: 0 !important; }

/* ── 모바일: 브랜드 영역 숨김 → 카드 중심 단일 컬럼 ── */
@media (max-width: 920px) {
    [data-testid="stColumn"]:has(.vtm-brand),
    [data-testid="column"]:has(.vtm-brand) {
        display: none !important;
    }
    [data-testid="stColumn"]:has(.vtm-login-marker),
    [data-testid="column"]:has(.vtm-login-marker) {
    ...
    .vtm-card-welcome { font-size: 1.5rem; }
}
</style>
""", unsafe_allow_html=True)
...
#    변경점: 레이아웃/스타일만 리디자인.
#            get_employees → selectbox(key="login_sel")
#            → text_input(key="login_pw") → button(key="btn_login")
#            → 비밀번호 검증 → session_state → wlog → st.rerun
#            로직은 원본과 100% 동일.
# ───────────────────────────────────────────────────────────────────
def render_login():
    inject_all()

    # ── Vimeo 배경 영상 + 어두운 오버레이 ──
    st.markdown(f"""
    <div class="vtm-bgvid">
      <iframe src="https://player.vimeo.com/video/{VTM_VIMEO_ID}?background=1&autoplay=1&loop=1&muted=1&autopause=0"
              frameborder="0"
              allow="autoplay; fullscreen"
              title="VTM OS Background"></iframe>
    </div>
    ...
    with right:
        # 카드 헤더 (marker 클래스로 이 컬럼 전체가 글래스 카드로 스타일링됨)
        st.markdown(f"""
        <div class="vtm-login-marker"></div>
        ...
        st.markdown("""
        <p class="vtm-ver"><b>VTM OS 2.0.0</b> · 개발자: 박동진 본부장</p>
        """, unsafe_allow_html=True)
