# vtm_dashboard.py  ← v4 달력 그리드 완전 재작성
...
.stButton>button {
    ...
    color: #000 !important; font-weight: 900 !important; font-size: 0.92rem !important;
...
.stButton>button * { color:#000 !important; font-weight:900 !important; }

/* vtm-card */
.vtm-card {
    background:#FFFFFF; border-radius:14px; padding:18px 22px; margin:8px 0;
    box-shadow:0 4px 18px rgba(0,0,0,0.25); border:1px solid #E2E8F0;
...
.met-card {
    ...
    border:1px solid #D4AF37; border-radius:14px; padding:18px 12px;
...
.topbar {
    ...
    border-bottom:2px solid #D4AF37; padding:12px 18px; border-radius:12px;
...
.stNumberInput>div>div>input {
    ...
    color:#111 !important; border:1.5px solid #C8A84B !important;
...
.stSelectbox>div>div {
    ...
    border:1.5px solid #C8A84B !important; border-radius:8px !important;
}
.stSelectbox * { color:#111 !important; font-weight:700 !important; }
.stDateInput>div>div>input {
    background:linear-gradient(135deg,#FFFFF0 0%,#FAEBD7 100%) !important;
    color:#111 !important; border:1.5px solid #C8A84B !important; border-radius:8px !important;
}
.stMultiSelect>div>div {
    background:linear-gradient(135deg,#FFFFF0 0%,#FFF8E7 100%) !important;
    border:1.5px solid #C8A84B !important; border-radius:8px !important;
}
.stMultiSelect * { color:#111 !important; font-weight:700 !important; }
...
.cal-hwd  { color: #D4AF37; }
.cal-hsat { color: #60A5FA; }
.cal-hsun { color: #F87171; }

/* 달력 셀 공통 — 버튼 리셋 */
.cal-cell-wd button,
.cal-cell-today button,
.cal-cell-sel button,
.cal-cell-sat button,
.cal-cell-sun button,
.cal-cell-empty button {
...
/* column 패딩 제거 */
.cal-row-wrap [data-testid="column"] {
    padding-left: 0 !important;
    padding-right: 0 !important;
}
.cal-row-wrap [data-testid="stHorizontalBlock"] {
    gap: 0 !important;
}
...
<canvas id="vtm-stars" style="position:fixed;top:0;left:0;
    width:100vw;height:100vh;pointer-events:none;z-index:0;opacity:0.45;"></canvas>
<script>
(function(){
    function boot(){
        var cv=document.getElementById('vtm-stars');
        if(!cv){setTimeout(boot,500);return;}
    ...
    with mid:
        ...
        <div style="margin-top:50px;background:rgba(15,23,42,0.92);
          ...
          <div style="text-align:center;margin-bottom:24px;">
            {logo_svg(76)}
            <h1 style="color:#D4AF37;font-size:1.5rem;font-weight:900;margin:12px 0 4px;">
                (주) 브이티엠
            </h1>
          ...
          <div style="background:rgba(212,175,55,0.12);border:1px solid rgba(212,175,55,0.3);
            ...
            <span style="color:#D4AF37;font-weight:900;font-size:0.84rem;">🔐 시스템 접속 인증</span>
          </div>
        ...
        <div style="text-align:center;margin-top:16px;">
          <p style="color:#475569;font-size:0.75rem;font-weight:700;">개발자: 박동진 본부장</p>
        </div>""", unsafe_allow_html=True)
        ...
        with cols[i]:
            ...
            if is_active and is_home:
                st.markdown(
                    ...
                    f'box-shadow:0 0 12px rgba(5,240,128,0.45);margin:2px 0;">{label}</div>',
                    unsafe_allow_html=True)
            elif is_active:
                st.markdown(
                    ...
                    f'box-shadow:0 0 10px rgba(212,175,55,0.45);margin:2px 0;">{label}</div>',
                    unsafe_allow_html=True)
            else:
...
def page_emp_attend():
    ...
    with c2:
        st.markdown(f"""<div class="vtm-card" style="text-align:center;">
          ...
          <p style="color:#64748B;">현재 KST: {now_kst().strftime('%H:%M')}</p>
        </div>""", unsafe_allow_html=True)
...
def page_emp_report():
    ...
    with tab2:
        ...
        st.markdown("""<div class='vtm-card'>
          <h3>🔗 산출물 링크</h3>
          <p style="font-size:0.8rem;color:#64748B;">완성본은 Google Drive에 날짜 폴더 생성 후 업로드해 주세요.</p>
        </div>""", unsafe_allow_html=True)
...
def render_day_detail(uid, d_str):
    conn = get_conn()
    att = pd.read_sql(
        "SELECT * FROM attendance WHERE emp_id=? AND work_date=? LIMIT 1",
        conn, params=(uid,d_str))
    rep = pd.read_sql(
        "SELECT * FROM reports WHERE emp_id=? AND work_date=? LIMIT 1",
        conn, params=(uid,d_str))
    conn.close()
    ...
    <div style="background:linear-gradient(90deg,#1E293B,#0F172A);
                ...
                padding:14px 22px;margin:10px 0 8px;position:relative;z-index:2;">
    ...
    cl, _ = st.columns([1,6])
    with cl:
        if st.button("✕  닫기", key="cal_close_btn", use_container_width=True):
    ...
    if not att.empty:
        ...
        with ac1:
            st.markdown(f"""
            <div class="vtm-card" style="text-align:center;padding:14px;">
              <h3 style="font-size:0.88rem;margin:0 0 6px;">🟢 출근 시간</h3>
              <p style="font-size:1.65rem;font-weight:900;color:#10B981;margin:4px 0;">{ci_t}</p>
              <p style="font-size:0.82rem;color:#64748B;margin:0;">{atp}</p>
            </div>""", unsafe_allow_html=True)
        with ac2:
            st.markdown(f"""
            <div class="vtm-card" style="text-align:center;padding:14px;">
              <h3 style="font-size:0.88rem;margin:0 0 6px;">🔴 퇴근 시간</h3>
              <p style="font-size:1.65rem;font-weight:900;color:#EF4444;margin:4px 0;">{co_t}</p>
              <p style="font-size:0.82rem;color:#64748B;margin:0;">&nbsp;</p>
            </div>""", unsafe_allow_html=True)
    else:
        st.markdown("""
        <div style="background:rgba(239,68,68,0.1);border:1px solid #EF4444;
          ...
          <span style="color:#EF4444;font-weight:900;font-size:0.95rem;">❗ 출근 기록 없음</span>
        </div>""", unsafe_allow_html=True)
    ...
    if not rep.empty:
        ...
        # 상태 색상
        sc_map = {"승인":"#DC2626","대기중":"#F59E0B","반려":"#DC2626","보류":"#8B5CF6"}
        ...
        if status == "승인":
            ...
            <div style="position:absolute;top:12px;right:16px;
                        transform:rotate(-20deg);opacity:0.92;z-index:10;pointer-events:none;">
              <svg width="82" height="82" viewBox="0 0 82 82" xmlns="http://www.w3.org/2000/svg">
                <circle cx="41" cy="41" r="38" fill="none" stroke="#DC2626" stroke-width="4.5"/>
                <circle cx="41" cy="41" r="30" fill="none" stroke="#DC2626" stroke-width="2"/>
                <text x="41" y="37" text-anchor="middle"
                      font-family="Noto Sans KR,Arial,sans-serif"
                      font-weight="900" font-size="14" fill="#DC2626" letter-spacing="3">승인</text>
                <text x="41" y="53" text-anchor="middle"
                      font-family="Arial,sans-serif"
                      font-weight="900" font-size="9" fill="#DC2626" letter-spacing="1.5">APPROVED</text>
              </svg>
            </div>"""
        elif status == "반려":
            stamp_html = """
            <div style="position:absolute;top:12px;right:16px;
              ...
              <svg width="82" height="82" viewBox="0 0 82 82" xmlns="http://www.w3.org/2000/svg">
                ...
                <text x="41" y="37" text-anchor="middle"
                      font-family="Noto Sans KR,Arial,sans-serif"
                      font-weight="900" font-size="14" fill="#DC2626" letter-spacing="3">반려</text>
                <text x="41" y="53" text-anchor="middle"
                      font-family="Arial,sans-serif"
                      font-weight="900" font-size="9" fill="#DC2626" letter-spacing="1.5">REJECTED</text>
              </svg>
            </div>"""
        elif status == "보류":
            stamp_html = """
            <div style="position:absolute;top:12px;right:16px;
                        transform:rotate(-20deg);opacity:0.88;z-index:10;pointer-events:none;">
              <svg width="82" height="82" viewBox="0 0 82 82" xmlns="http://www.w3.org/2000/svg">
                <circle cx="41" cy="41" r="38" fill="none" stroke="#8B5CF6" stroke-width="4.5"/>
                <circle cx="41" cy="41" r="30" fill="none" stroke="#8B5CF6" stroke-width="2"/>
                <text x="41" y="37" text-anchor="middle"
                      font-family="Noto Sans KR,Arial,sans-serif"
                      font-weight="900" font-size="14" fill="#8B5CF6" letter-spacing="3">보류</text>
                <text x="41" y="53" text-anchor="middle"
                      font-family="Arial,sans-serif"
                      font-weight="900" font-size="9" fill="#8B5CF6" letter-spacing="1.5">PENDING</text>
              </svg>
            </div>"""

        pm_rem_html = (
            f'<p style="font-size:0.8rem;color:#475569;margin:4px 0 0;">'
            f'💬 특이사항: {pm_rem}</p>'
            if pm_rem and pm_rem != "—" else "")

        # ── 보고서 카드 (순수 HTML — unsafe_allow_html 단일 호출) ──
        st.markdown(f"""
        <div class="vtm-card" style="position:relative;padding:16px 20px;margin:10px 0 6px;">
          {stamp_html}
          <div style="display:flex;justify-content:space-between;
                      align-items:center;margin-bottom:12px;">
            <span style="font-size:1rem;font-weight:900;color:#1E293B;">📝 업무 보고서</span>
            <span style="background:{sc};color:#fff;padding:4px 18px;
                         border-radius:20px;font-weight:900;font-size:0.83rem;">
                {s_emoji} {status}
            </span>
          </div>
          ...
          <div style="background:#F8FAFC;border:1px solid #E2E8F0;
            ...
            <p style="font-size:0.72rem;color:#64748B;font-weight:700;margin:0 0 5px;">
                🌅 오전 업무 계획
            </p>
            <p style="font-size:0.9rem;font-weight:700;color:#1E293B;
                      margin:0 0 4px;white-space:pre-wrap;">{am_tasks}</p>
            <p style="font-size:0.78rem;color:#475569;margin:0;">
                우선순위: {am_pri}&nbsp;&nbsp;|&nbsp;&nbsp;특이사항: {am_notes}
            </p>
          </div>
          ...
          <div style="background:#F0FDF4;border:1px solid #BBF7D0;
            ...
            <p style="font-size:0.9rem;font-weight:700;color:#1E293B;
                      margin:0 0 7px;white-space:pre-wrap;">{pm_done}</p>
            <div style="background:#E2E8F0;border-radius:6px;height:8px;margin:4px 0 5px;">
              <div style="background:{bar_c};width:{prg}%;height:8px;border-radius:6px;"></div>
            </div>
            <p style="font-size:0.76rem;color:#475569;margin:0;">
                진행률 {prg}%&nbsp;&nbsp;|&nbsp;&nbsp;내일 예정: {pm_tom}
            </p>
            {pm_rem_html}
          </div>
        </div>""", unsafe_allow_html=True)

        # 링크 (별도 markdown — anchor 태그 허용)
        dl_a = (f'<a href="{dl_val}" target="_blank" '
                f'style="color:#3B82F6;font-weight:700;text-decoration:none;">🔗 Drive 열기</a>'
                if dl_val else "없음")
        rl_a = (f'<a href="{rl_val}" target="_blank" '
                f'style="color:#3B82F6;font-weight:700;text-decoration:none;">🔗 결과물 열기</a>'
                if rl_val else "없음")
        st.markdown(f"""
        <div style="background:#F8F9FA;border:1px solid #E2E8F0;border-radius:8px;
                    padding:9px 16px;margin:4px 0;font-size:0.8rem;color:#475569;">
            📁 {dl_a}&nbsp;&nbsp;&nbsp;&nbsp;{rl_a}
        </div>""", unsafe_allow_html=True)
        ...
        if cmt:
            ...
            <div style="background:linear-gradient(135deg,#FFF8E7,#FFF3CD);
              ...
              <p style="font-size:0.74rem;color:#92610A;font-weight:900;margin:0 0 5px;">
                  💬 관리자 코멘트
              </p>
              <p style="font-size:0.92rem;font-weight:700;color:#1A1A1A;
                        margin:0;line-height:1.6;">{cmt}</p>
            </div>""", unsafe_allow_html=True)

        # 타임스탬프
        sub_d  = sub_at[-17:-3]  if len(sub_at)>13  else sub_at
        apr_d  = appr_at[-17:-3] if len(appr_at)>13 else appr_at
        tail   = f"제출: {sub_d}"
        if appr_at and appr_at != "—":
            tail += f"&nbsp;&nbsp;·&nbsp;&nbsp;처리: {apr_d}"
        st.markdown(f"""
        <div style="font-size:0.7rem;color:#94A3B8;text-align:right;margin:5px 0 10px;">
            {tail}
        </div>""", unsafe_allow_html=True)

    else:
        st.markdown("""
        <div style="background:rgba(100,116,139,0.1);border:1px solid #334155;
                    border-radius:12px;padding:20px;text-align:center;margin:6px 0;">
          <p style="color:#64748B;font-weight:700;font-size:0.95rem;margin:0;">
              📭 이 날 업무 보고 내역이 없습니다.
          </p>
        </div>""", unsafe_allow_html=True)

    st.markdown("<hr style='border-color:#1E3A5F;margin:14px 0 6px;'>",
                unsafe_allow_html=True)
...
def page_emp_calendar():
    ...
    with c1:
        yr = st.number_input("연도", value=today.year, min_value=2024, max_value=2030, key="cy")
    with c2:
        mo = st.number_input("월",   value=today.month, min_value=1,   max_value=12,   key="cm")
    yr = int(yr); mo = int(mo)
    ...
    cal_weeks = calendar.monthcalendar(yr, mo)   # 월(0)~일(6)
    sel       = st.session_state.cal_selected

    # 월 타이틀
    month_kr = ["1월","2월","3월","4월","5월","6월",
                "7월","8월","9월","10월","11월","12월"][mo-1]
    st.markdown(f"""
    <div style="background:linear-gradient(90deg,#1E293B,#0F172A);
                border:1.5px solid #D4AF37;border-radius:12px;
                padding:12px 20px;margin:8px 0 0;text-align:center;">
      <span style="color:#D4AF37;font-size:1.15rem;font-weight:900;">
          📅 {yr}년 {month_kr}
      </span>
    </div>""", unsafe_allow_html=True)
    ...
    <div class="cal-header-row">
      <div class="cal-hcell cal-hwd">월</div>
      <div class="cal-hcell cal-hwd">화</div>
      <div class="cal-hcell cal-hwd">수</div>
      <div class="cal-hcell cal-hwd">목</div>
      <div class="cal-hcell cal-hwd">금</div>
    ...
    for w_idx, week in enumerate(cal_weeks):
        ...
        st.markdown(f'<div class="cal-row-wrap" id="wk{w_idx}">',
                    unsafe_allow_html=True)
        ...
        for col_i, day in enumerate(week):
            ...
            if day == 0:
                ...
                is_sl = (ds == sel)
                if is_sl:   ctype = "sel"
                elif is_td: ctype = "today"
                else:       ctype = "wd"

            with cols[col_i]:
                # CSS 클래스 래퍼
                st.markdown(f'<div class="cal-cell-{ctype}">',
                            unsafe_allow_html=True)

                # ── 빈 셀 ──
                if day == 0:
                    st.button(" ", key=f"ce_{w_idx}_{col_i}",
                              disabled=True, use_container_width=True)

                # ── 토/일 셀 (클릭 불가) ──
                elif is_sat or is_sun:
                    ds_wk  = f"{yr}-{mo:02d}-{day:02d}"
                    lbl_wk = str(day)
                    if ds_wk in att_map:
                        lbl_wk += "\n✅ 출근"
                    st.button(lbl_wk, key=f"cw_{w_idx}_{col_i}_{day}",
                              disabled=True, use_container_width=True)
                ...
                # ── 평일 셀 ──
                    ...
                    # 날짜 줄
                    mark   = ("★" if is_td else "") + ("◀" if is_sl else "")
                    l1     = f"{day} {mark}".strip()

                    # 출근 줄
                    if has_a:
                        atp_s = (att_map[ds]["att_type"] or "출근")[:4]
                        l2    = f"✅ {atp_s}"
                    else:
                        l2    = "○ 미출근"
                    ...
                    # 보고 줄
                    if has_r:
                        rs  = str(rep_map[ds]["status"] or "")
                        prg = rep_map[ds]["pm_progress"] or 0
                        l3  = {"승인": f"🔴 승인 {prg}%",
                               "반려": "❌ 반려",
                               "보류": "⏸ 보류",
                               "대기중": "⏳ 대기중"}.get(rs, f"📋 {rs}")
                    ...
                    if st.button(label, key=f"cd_{ds}",
                                 use_container_width=True):
                        st.session_state.cal_selected = None if is_sl else ds
                        st.rerun()

                st.markdown('</div>', unsafe_allow_html=True)

        st.markdown('</div>', unsafe_allow_html=True)   # cal-row-wrap 닫기

        # ── 이 주에 선택된 날짜 → 바로 아래 상세 펼침 ──
        week_wd = [
            f"{yr}-{mo:02d}-{d:02d}"
            for ci2, d in enumerate(week)
            if d != 0 and ci2 < 5
        ]
        if sel in week_wd:
            st.markdown('<div class="detail-bar"></div>', unsafe_allow_html=True)
        ...
        st.markdown(f"""<div class="vtm-card" style="padding:12px;margin:3px 0;">
          <div style="display:flex;justify-content:space-between;
            ...
            <div style="display:flex;gap:10px;flex-wrap:wrap;font-weight:700;font-size:0.84rem;">
              ...
              <span>📋 {atp}</span>
              <span style="color:{rs_c};">{'미제출(출근전)' if rs=='미제출' else '보고:'+rs}</span>
              <span>📊{prg}%</span>
            </div>
          </div>
        </div>""", unsafe_allow_html=True)
...
def page_admin_tasks():
    ...
    for _,r in reps.iterrows():
        ...
        dl_html=(f'<a href="{dl_val}" target="_blank" style="color:#3B82F6;font-weight:700;">링크열기</a>'
                 if dl_val else "없음")
        rl_html=(f'<a href="{rl_val}" target="_blank" style="color:#3B82F6;font-weight:700;">링크열기</a>'
                 if rl_val else "없음")
        am_txt=safe_str(r["am_tasks"]) or "미입력"
        pm_txt=safe_str(r["pm_done"])  or "미입력"
        cmt_v =safe_str(r["admin_comment"]) or ""
        prg_v =r["pm_progress"] if safe_str(str(r["pm_progress"])) else 0
        parts=[
            '<div class="vtm-card">',
            ...
            '  </div>',
            f'  <p><b>🌅 오전 계획:</b> {am_txt}</p>',
            f'  <p><b>🌇 완료 업무:</b> {pm_txt}</p>',
            f'  <p><b>📊 진행률:</b> {prg_v}%</p>',
            f'  <p><b>📁 Drive:</b> {dl_html} &nbsp;&nbsp; <b>🔗 결과물:</b> {rl_html}</p>',
        ]
        if cmt_v:
            parts.append(
                f'  <p style="background:rgba(212,175,55,0.15);border-left:3px solid #D4AF37;'
                f'padding:6px 10px;border-radius:4px;"><b>💬 코멘트:</b> {cmt_v}</p>')
        parts.append('</div>')
        st.markdown("\n".join(parts), unsafe_allow_html=True)

        cmt_input=st.text_input("💬 코멘트 입력 (선택사항)",key=f"tcmt_{rid}",
...
def page_admin_approve():
    ...
    st.markdown(f"<div class='vtm-card'><h3>📋 대기 {len(pend)}건</h3></div>",
                unsafe_allow_html=True)
...
def page_admin_emp():
    ...
    for _,emp in emp_df.iterrows():
        ...
        with ci:
            st.markdown(f"""
            <div class="vtm-card" style="padding:9px 15px;margin:2px 0;">
        ...
        with cb:
            if emp["active"] and not emp["is_admin"]:
                if st.button("🗑 퇴직",key=f"del_{emp['id']}",use_container_width=True):
                    c2=get_conn(); c2.execute("UPDATE employees SET active=0 WHERE id=?",(emp["id"],))
                    c2.commit(); c2.close()
                ...
                if st.button("♻ 복직",key=f"act_{emp['id']}",use_container_width=True):
                    c2=get_conn(); c2.execute("UPDATE employees SET active=1 WHERE id=?",(emp["id"],))
                    c2.commit(); c2.close()
...
def page_admin_excel():
    ...
    if st.button("📥  엑셀 생성",key="btn_excel",use_container_width=True):
        ...
        if "출퇴근 기록" in rec_types:
            ...
            if not df.empty:
                df.columns=["직원명","날짜","유형","출근","퇴근"]; sheets["출퇴근 기록"]=df
