...
# ═══════════════════════════════════════════════════════════════════
#  VTM OS 로그인 — 안정화 버전 패치 (v2.0.1-stable)
#
#  [이전 버전 대비 변경점 — 배포 안정화]
#   · Vimeo iframe 배경 완전 제거 (vtm-bgvid 관련 HTML/CSS 전부 삭제)
#   · 배경: CSS radial-gradient + linear-gradient 만 사용
#   · :has() 선택자 전면 제거
#   · 로그인 카드: st.container(key="vtm_login_card") 가 만들어 주는
#     클래스 기반 wrapper div( .st-key-vtm_login_card )로 스타일 적용
#     ※ Streamlit ≥ 1.39 필요 (Streamlit Cloud 기본 버전은 충족)
#   · Human × AI Workforce 브랜드 카피 + Workforce Status 패널 추가
#     (실제 기능 없는 순수 상태 표시 카드)
#
#  [불변 영역]
#   · 로그인 로직 / Supabase / get_employees / session_state / st.rerun
#     → 원본과 100% 동일, 단 한 줄도 변경 없음
#
...
""", unsafe_allow_html=True)
    ...
    # ═══════════════════════════════════════════════════════════
    # ▼▼▼ [로그인 전용 CSS — 안정화 버전] ▼▼▼
    #  · Vimeo iframe 관련 CSS 제거
    #  · 배경: radial-gradient + linear-gradient 만 사용
    #  · :has() 미사용 — 카드는 .st-key-vtm_login_card (클래스 기반),
    #    모바일 브랜드 숨김은 :first-child 위치 선택자 사용
    #  · 로그인 전(logged_in=False)에만 주입 → 로그인 후 영향 없음
    # ═══════════════════════════════════════════════════════════
    if not st.session_state.get("logged_in", False):
        st.markdown("""
<style>
/* ── 로그인 배경: CSS 그라데이션만 사용 (외부 리소스 없음) ── */
.stApp {
    background:
        ...
        radial-gradient(700px 420px at 60% 10%, rgba(45,212,191,0.06) 0%, transparent 65%),
...
#    · Vimeo iframe 제거 (배경은 위 CSS 그라데이션이 담당)
#    · 카드: st.container(key="vtm_login_card") → 클래스 기반 스타일
#    · 브랜드 영역: Human × AI Workforce 카피 + Workforce 패널 추가
#    · 로그인 로직: 원본과 100% 동일
# ───────────────────────────────────────────────────────────────────
def render_login():
    inject_all()
    ...
    with right:
        ...
        with card:
            ...
            st.markdown("""
            <p class="vtm-ver"><b>VTM OS 2.0.1</b> · 개발자: 박동진 본부장</p>
            """, unsafe_allow_html=True)
