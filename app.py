# vtm_dashboard.py  ← 실제 달력 그리드 + 빨간 도장 + 흰색 평일 셀 v3
...
/* 평일 셀 — 흰색 배경 */
div[data-cal="wd"] .stButton>button,
div[data-cal="today"] .stButton>button,
div[data-cal="sel"] .stButton>button {{
...
div[data-cal="wd"] .stButton>button:hover {{
...
div[data-cal="today"] .stButton>button:hover {{
...
div[data-cal="sel"] .stButton>button:hover {{
...
/* 토요일 */
div[data-cal="sat"] .stButton>button {{
    background: #EFF6FF !important;
...
div[data-cal="sat"] .stButton>button:hover {{
    transform: none !important;
    box-shadow: none !important;
}}

/* 일요일 */
div[data-cal="sun"] .stButton>button {{
...
div[data-cal="sun"] .stButton>button:hover {{
    transform: none !important;
    box-shadow: none !important;
}}

/* 빈 셀 */
div[data-cal="empty"] .stButton>button {{
...
div[data-cal="empty"] .stButton>button:hover {{
    transform: none !important;
    box-shadow: none !important;
}}
...
/* column gap 제거 (달력 셀 붙이기) */
.cal-cols [data-testid="stHorizontalBlock"] {{
    gap: 0 !important;
}}
.cal-cols [data-testid="column"] {{
    padding: 0 !important;
    gap: 0 !important;
}}
...
def render_day_detail(uid, d_str):
    ...
    <div style="background:linear-gradient(90deg,#1E293B,#0F172A);
                ...
                padding:14px 22px;margin:10px 0 8px;">
    ...
    if not att.empty:
        ...
        <div style="background:rgba(239,68,68,0.1);border:1px solid #EF4444;
          ...
          <span style="color:#EF4444;font-weight:900;font-size:0.95rem;">
              ❗ 출근 기록 없음
          </span>
        </div>""", unsafe_allow_html=True)
    ...
    if not rep.empty:
        ...
        sc      = {"승인":"#DC2626","대기중":"#F59E0B","반려":"#EF4444","보류":"#8B5CF6"}.get(status,"#6B7280")
        ...
        if status == "승인":
            ...
            <div style="position:absolute;top:14px;right:20px;
                        transform:rotate(-20deg);opacity:0.90;z-index:10;
                        pointer-events:none;">
              <svg width="80" height="80" viewBox="0 0 80 80"
                   xmlns="http://www.w3.org/2000/svg">
                <circle cx="40" cy="40" r="36"
                        fill="none" stroke="#DC2626" stroke-width="4.5"/>
                <circle cx="40" cy="40" r="29"
                        fill="none" stroke="#DC2626" stroke-width="2"/>
                <text x="40" y="35" text-anchor="middle"
                      font-family="Noto Sans KR,Arial,sans-serif"
                      font-weight="900" font-size="13" fill="#DC2626"
                      letter-spacing="2">승인</text>
                <text x="40" y="51" text-anchor="middle"
                      font-family="Arial,sans-serif"
                      font-weight="900" font-size="9" fill="#DC2626"
                      letter-spacing="1">APPROVED</text>
              </svg>
            </div>"""
        elif status == "반려":
            stamp_html = """
            <div style="position:absolute;top:14px;right:20px;
              ...
              <svg width="80" height="80" viewBox="0 0 80 80"
                ...
                <text x="40" y="35" text-anchor="middle"
                      font-family="Noto Sans KR,Arial,sans-serif"
                      font-weight="900" font-size="13" fill="#DC2626"
                      letter-spacing="2">반려</text>
                <text x="40" y="51" text-anchor="middle"
                      font-family="Arial,sans-serif"
                      font-weight="900" font-size="9" fill="#DC2626"
                      letter-spacing="1">REJECTED</text>
              </svg>
            </div>"""
        elif status == "보류":
            stamp_html = """
            <div style="position:absolute;top:14px;right:20px;
                        transform:rotate(-20deg);opacity:0.85;z-index:10;
                        pointer-events:none;">
              <svg width="80" height="80" viewBox="0 0 80 80"
                   xmlns="http://www.w3.org/2000/svg">
                <circle cx="40" cy="40" r="36"
                        fill="none" stroke="#8B5CF6" stroke-width="4.5"/>
                <circle cx="40" cy="40" r="29"
                        fill="none" stroke="#8B5CF6" stroke-width="2"/>
                <text x="40" y="35" text-anchor="middle"
                      font-family="Noto Sans KR,Arial,sans-serif"
                      font-weight="900" font-size="13" fill="#8B5CF6"
                      letter-spacing="2">보류</text>
                <text x="40" y="51" text-anchor="middle"
                      font-family="Arial,sans-serif"
                      font-weight="900" font-size="9" fill="#8B5CF6"
                      letter-spacing="1">PENDING</text>
              </svg>
            </div>"""

        # ── 보고서 메인 카드 ──
        pm_rem_html = (f'<p style="font-size:0.8rem;color:#475569;margin:4px 0 0;">'
                       f'💬 특이사항: {pm_rem}</p>'
        ...
        <div class="vtm-card" style="position:relative;padding:16px 20px;margin:10px 0 6px;">
          ...
          <div style="display:flex;justify-content:space-between;
            ...
            <span style="font-size:1.02rem;font-weight:900;color:#1E293B;">
                📝 업무 보고서
            </span>
          ...
          <div style="background:#F0FDF4;border:1px solid #BBF7D0;
            ...
            <div style="background:#E2E8F0;border-radius:6px;
                        height:8px;margin:4px 0 5px;">
              <div style="background:{bar_c};width:{prg}%;
        ...
        dl_a = (f'<a href="{dl_val}" target="_blank" '
                f'style="color:#3B82F6;font-weight:700;text-decoration:none;">'
                f'🔗 Drive 열기</a>' if dl_val else '없음')
        rl_a = (f'<a href="{rl_val}" target="_blank" '
                f'style="color:#3B82F6;font-weight:700;text-decoration:none;">'
                f'🔗 결과물 열기</a>' if rl_val else '없음')
        st.markdown(f"""
        ...
        <div style="font-size:0.7rem;color:#94A3B8;
                    text-align:right;margin:5px 0 10px;">{tail}</div>
        """, unsafe_allow_html=True)
    ...
    st.markdown(
        "<hr style='border-color:#1E3A5F;margin:14px 0 6px;'>",
        unsafe_allow_html=True)
...
def page_emp_calendar():
    ...
    with c1:
        yr = st.number_input("연도", value=today.year,
                             min_value=2024, max_value=2030, key="cy")
    with c2:
        mo = st.number_input("월", value=today.month,
                             min_value=1, max_value=12, key="cm")
    yr = int(yr); mo = int(mo)

    # ── DB 조회 ──
    conn = get_conn()
    att_df = pd.read_sql(
        "SELECT work_date,att_type FROM attendance "
        "WHERE emp_id=? AND work_date LIKE ?",
        conn, params=(uid, f"{yr}-{mo:02d}-%"))
    rep_df = pd.read_sql(
        "SELECT work_date,status,pm_progress FROM reports "
        "WHERE emp_id=? AND work_date LIKE ?",
        conn, params=(uid, f"{yr}-{mo:02d}-%"))
    conn.close()

    att_map = ({r["work_date"]: r for _, r in att_df.iterrows()}
               if not att_df.empty else {})
    rep_map = ({r["work_date"]: r for _, r in rep_df.iterrows()}
               if not rep_df.empty else {})

    cal_weeks = calendar.monthcalendar(yr, mo)   # 월~일
    ...
    <div class="cal-dow-row">
      <div class="cal-dow-cell cal-dow-wd">월</div>
      <div class="cal-dow-cell cal-dow-wd">화</div>
      <div class="cal-dow-cell cal-dow-wd">수</div>
      <div class="cal-dow-cell cal-dow-wd">목</div>
      <div class="cal-dow-cell cal-dow-wd">금</div>
      <div class="cal-dow-cell cal-dow-sat">토</div>
      <div class="cal-dow-cell cal-dow-sun">일</div>
    </div>""", unsafe_allow_html=True)

    # ── 주(週) 단위 렌더링 ──
    for w_idx, week in enumerate(cal_weeks):

        # cal-row div: column gap 제거용
        st.markdown(f'<div class="cal-cols" id="week-{w_idx}">', unsafe_allow_html=True)
        ...
        for i, day in enumerate(week):
            ...
            if day == 0:
                ...
                d_str2 = f"{yr}-{mo:02d}-{day:02d}"
                is_td  = (d_str2 == today_str())
                is_sel = (d_str2 == sel)
            ...
            with cols[i]:
                # data-cal 속성으로 CSS 타겟팅
                st.markdown(
                    f'<div data-cal="{cell_type}">',
                    unsafe_allow_html=True)
                ...
                # ── 토/일 셀 (클릭 비활성) ──
                    ...
                    d_wk    = f"{yr}-{mo:02d}-{day:02d}"
                    has_wk  = d_wk in att_map
                    wk_lbl  = f"{day}\n{'✅ 출근' if has_wk else ''}"
                    st.button(
                        wk_lbl, key=f"cw_{w_idx}_{i}_{day}",
                        use_container_width=True, disabled=True)

                # ── 평일 셀 ──
                else:
                    d_str2  = f"{yr}-{mo:02d}-{day:02d}"
                    has_att = d_str2 in att_map
                    has_rep = d_str2 in rep_map
                    rep_st  = rep_map[d_str2]["status"]      if has_rep else None
                    rep_prg = rep_map[d_str2]["pm_progress"] if has_rep else 0
                    is_td   = (d_str2 == today_str())
                    is_sel  = (d_str2 == sel)

                    # 라벨 3줄 구성
                    mark       = (" ★" if is_td else "") + (" ◀" if is_sel else "")
                    line_day   = f"{day}{mark}"

                    if has_att:
                        atp_s   = (att_map[d_str2]["att_type"] or "출근")[:4]
                        line_att = f"✅ {atp_s}"
                    else:
                        line_att = "○ 미출근"

                    if has_rep:
                        rep_st_str = str(rep_st) if rep_st else ""
                        st_lbl = {
                            "승인":  f"🔴 승인 {rep_prg}%",
                            "반려":  "❌ 반려",
                            "보류":  "⏸ 보류",
                            "대기중":f"⏳ 대기중",
                        }.get(rep_st_str, f"📋 {rep_st_str}")
                        line_rep = st_lbl
                    else:
                        line_rep = "— 보고없음"
                    ...
                    if st.button(label, key=f"cd_{d_str2}",
                                 use_container_width=True):
                        # 같은 날 재클릭 → 닫기
                        st.session_state.cal_selected = (
                            None if is_sel else d_str2)
                        st.rerun()

                st.markdown('</div>', unsafe_allow_html=True)

        st.markdown('</div>', unsafe_allow_html=True)  # cal-cols 닫기

        # ── 이 주(週)에 선택된 날짜가 있으면 바로 아래에 상세 펼침 ──
        week_wd_dates = [
            f"{yr}-{mo:02d}-{day:02d}"
            for idx, day in enumerate(week)
            if day != 0 and idx < 5           # 평일(월~금)만
        ]
        if sel in week_wd_dates:
            st.markdown(
                '<div class="detail-entry-bar"></div>',
                unsafe_allow_html=True)
...
def page_admin_tasks():
    ...
    def do_approve_task(rid, status, emp, comment):
        c2 = get_conn()
        c2.execute(
            "UPDATE reports SET status=?,admin_comment=?,approved_at=? WHERE id=?",
    ...
    for _, r in reps.iterrows():
        ...
        dl_val  = safe_str(r["drive_link"])
        rl_val  = safe_str(r["result_link"])
        dl_html = (f'<a href="{dl_val}" target="_blank" '
                   f'style="color:#3B82F6;font-weight:700;">링크열기</a>'
                   if dl_val else "없음")
        rl_html = (f'<a href="{rl_val}" target="_blank" '
                   f'style="color:#3B82F6;font-weight:700;">링크열기</a>'
                   if rl_val else "없음")
        ...
        card_parts = [
            ...
            f'    <span style="background:{sc};color:#fff;padding:3px 14px;'
            f'border-radius:12px;font-weight:900;font-size:0.8rem;">{status}</span>',
            '  </div>',
            f'  <p><b>🌅 오전 계획:</b> {am_tasks_txt}</p>',
            f'  <p><b>🌇 완료 업무:</b> {pm_done_txt}</p>',
            f'  <p><b>📊 진행률:</b> {prg_val}%</p>',
            f'  <p><b>📁 Drive:</b> {dl_html} &nbsp;&nbsp;'
            f'<b>🔗 결과물:</b> {rl_html}</p>',
        ]
        if cmt_val:
            card_parts.append(
                f'  <p style="background:rgba(212,175,55,0.15);'
                f'border-left:3px solid #D4AF37;padding:6px 10px;border-radius:4px;">'
                f'<b>💬 코멘트:</b> {cmt_val}</p>')
        ...
        with ba:
            if st.button("✅ 승인", key=f"tap_{rid}", use_container_width=True):
                do_approve_task(rid, "승인", r["emp_name"],
                                cmt_input or "승인되었습니다.")
        with bb:
            if st.button("❌ 반려", key=f"trj_{rid}", use_container_width=True):
                do_approve_task(rid, "반려", r["emp_name"],
                                cmt_input or "반려 사유를 입력해 주세요.")
        with bc:
            if st.button("⏸ 보류", key=f"thl_{rid}", use_container_width=True):
                do_approve_task(rid, "보류", r["emp_name"],
                                cmt_input or "보류 처리되었습니다.")
...
def page_admin_approve():
    ...
    conn = get_conn()
    pend = pd.read_sql(
        "SELECT * FROM reports WHERE status='대기중' ORDER BY submitted_at DESC",
        conn)
    conn.close()
    if pend.empty:
        ...
        return
    st.markdown(
        f"<div class='vtm-card'><h3>📋 대기 {len(pend)}건</h3></div>",
        unsafe_allow_html=True)
    ...
    for _, r in pend.iterrows():
        with st.expander(
                f"📝 {r['emp_name']}  ·  {r['work_date']}  ·  {r['pm_progress']}%"):
            st.markdown(f"""
...
""")
            ...
            with ca:
                if st.button("✅ 승인", key=f"ap_{r['id']}",
                             use_container_width=True):
                    do_approve(r['id'], "승인", r['emp_name'],
                               cmt or "승인되었습니다.")
            with cb:
                if st.button("❌ 반려", key=f"rj_{r['id']}",
                             use_container_width=True):
                    do_approve(r['id'], "반려", r['emp_name'],
                               cmt or "반려 사유를 입력해 주세요.")
            with cc:
                if st.button("⏸ 보류", key=f"hl_{r['id']}",
                             use_container_width=True):
                    do_approve(r['id'], "보류", r['emp_name'],
                               cmt or "보류 처리되었습니다.")
...
def page_admin_emp():
    ...
    st.markdown(
        "<div class='vtm-card'><h3>👥 전체 직원 목록</h3></div>",
        unsafe_allow_html=True)
    for _, emp in emp_df.iterrows():
        ...
        with cb:
            if emp["active"] and not emp["is_admin"]:
                if st.button("🗑 퇴직", key=f"del_{emp['id']}",
                             use_container_width=True):
                    c2 = get_conn()
                    c2.execute("UPDATE employees SET active=0 WHERE id=?",
                               (emp["id"],))
                    c2.commit(); c2.close()
                    wlog("EMP_DEL", st.session_state.user_name, emp["name"])
                    st.success(f"'{emp['name']}' 퇴직 처리"); st.rerun()
            elif not emp["active"]:
                if st.button("♻ 복직", key=f"act_{emp['id']}",
                             use_container_width=True):
                    c2 = get_conn()
                    c2.execute("UPDATE employees SET active=1 WHERE id=?",
                               (emp["id"],))
                    c2.commit(); c2.close()
    ...
    st.markdown("---")
    st.markdown(
        "<div class='vtm-card'><h3>➕ 신규 직원 등록</h3></div>",
        unsafe_allow_html=True)
    with st.form("form_emp", clear_on_submit=True):
        ...
        if st.form_submit_button("✅  직원 등록", use_container_width=True):
            if not new_name.strip() or not new_role.strip():
                ...
                new_id = ("emp_"
                          + re.sub(r'[^a-z0-9]', '', new_name.lower())
                          + str(int(time.time()))[-5:])
                try:
                    c2 = get_conn()
                    c2.execute(
                        "INSERT INTO employees"
                        "(id,name,role,is_admin,password,active,created_at)"
                        " VALUES(?,?,?,?,?,1,?)",
...
def page_admin_excel():
    topbar("📥 엑셀 다운로드")
    st.markdown(
        "<div class='vtm-card'><h3>📥 다운로드 조건 설정</h3></div>",
        unsafe_allow_html=True)
    ...
    with c2:
        period    = st.selectbox("📅 기간",
                                 ["날짜별","주간","월간","전체"], key="ex_period")
        ...
        if period == "날짜별":
            ...
            st.markdown(
                f"<span style='color:#D4AF37;font-weight:700;font-size:0.84rem;'>"
                f"📅 {ws} ~ {we}</span>", unsafe_allow_html=True)
        elif period == "월간":
            my = st.number_input("연도", value=kst_today.year,
                                 min_value=2024, max_value=2030, key="ex_my")
            mm = st.number_input("월",   value=kst_today.month,
                                 min_value=1,    max_value=12,   key="ex_mm")
    ...
    if st.button("📥  엑셀 생성", key="btn_excel", use_container_width=True):
        ...
        if "출퇴근 기록" in rec_types:
            ...
            p = [d_from, d_to]
            if sel_emp != "전체 인원":
                q += " AND emp_name=?"; p.append(sel_emp)
            df = pd.read_sql(q + " ORDER BY work_date,emp_name", conn, params=p)
            if not df.empty:
                df.columns = ["직원명","날짜","유형","출근","퇴근"]
                sheets["출퇴근 기록"] = df
        if "업무 보고" in rec_types:
            q = ("SELECT emp_name,work_date,am_tasks,am_priority,pm_done,"
                 "pm_progress,pm_tomorrow,pm_remarks,drive_link,result_link,"
                 "status,admin_comment,submitted_at"
            ...
            if not df.empty:
                df.columns = ["직원명","날짜","오전계획","우선순위","완료업무",
                              "진행률","내일예정","특이사항","Drive","결과링크",
                              "상태","코멘트","제출시간"]
                sheets["업무 보고"] = df
        conn.close()
        if not sheets:
            st.warning("해당 조건 데이터 없음"); return
